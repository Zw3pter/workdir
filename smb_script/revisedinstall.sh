...
# Installer Samba, Winbind og Kerberos-pakker
sudo dnf install samba samba-client samba-common krb5-workstation krb5-libs samba-winbind samba-winbind-clients -y

# Start og aktiver Samba og Winbind services
sudo systemctl start smb nmb winbind
sudo systemctl enable smb nmb winbind

# Tilslut til AD-domænet
echo "Tilslutter til AD-domænet. Indtast administrator password:"
...
[faelles]
	path = /samba/faelles
	valid users = @"DCTRL-LTJ\faelles"
	writable = yes
	browsable = yes

[lager]
   path = /samba/lager
	valid users = @"DCTRL-LTJ\lager"
   writable = yes
   browsable = yes

[regnskab]
   path = /samba/regnskab
   valid users = @"DCTRL-LTJ\regnskab"
   writable = yes
   browsable = yes
EOF'

# Genstart Samba og Winbind services
...
# Tilføj Samba, Winbind og Kerberos til firewall og genindlæs
